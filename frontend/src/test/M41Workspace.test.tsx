...
it("makes upload analysis primary and uses read-only server configuration without benchmark metrics", async () => {
  ...
  render(<App />);
  expect(await screen.findByRole("heading", { name: "Choose your dossier" })).toBeVisible();
  expect(screen.getByText(/prospective forward compatibility, which is not currently operational/i)).toBeVisible();
  expect(screen.getByText(/have not been validated by a regulatory expert/i)).toBeVisible();
