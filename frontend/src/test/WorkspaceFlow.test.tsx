...
const catalog = fixture.configuration;
const inventory = { ...fixture.inventory, leaves: fixture.inventory.leaves.slice(0, 1) };
...
it("replaces setup with the loader and restores editable inputs after a rejected upload", async () => {
  ...
  vi.stubGlobal("fetch", vi.fn((url: string) => url.endsWith("/config/product") ? Promise.resolve(json(catalog)) : new Promise<Response>((resolve) => { resolveUpload = resolve; })));
  mount();
...
it("submits shared target intent without a model override, recovers polling without restarting, and focuses results", async () => {
  sessionStorage.setItem("regbridge.inventory", JSON.stringify(inventory));
  let polls = 0;
  const fetchMock = vi.fn((url: string, options?: RequestInit) => {
    if (url.endsWith("/config/product")) return Promise.resolve(json(catalog));
