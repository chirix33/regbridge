...
test("M4 guided journey and dashboard are accessible", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "About RegBridge" })).toBeVisible();

  await page.goto("/demo/case-a");
...
test("M4.2 uploads the public-standards ZIP and compares package-derived inputs", async ({ page }) => {
  ...
  await expect(page.getByText(/have not been validated by a regulatory expert/i)).toBeVisible();
  ...
  await caseA.getByText("How this conclusion is supported", { exact: true }).click();
