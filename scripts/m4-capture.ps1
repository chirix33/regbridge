...
$verificationDatabase = Join-Path $repositoryRoot "results\.pytest-m4-capture-regbridge.sqlite3"
$apiProcess = $null
...
try {
    ...
    try {
        & npm --prefix frontend run capture:m4
        if ($LASTEXITCODE -ne 0) { throw "M4 screenshot capture failed." }
    }
    finally {
        Pop-Location
    }
...
Write-Output "M4 screenshots captured under paper/figures/m4."
