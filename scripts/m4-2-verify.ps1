...
Push-Location $root
try {
    & $python -m app.product.verify_m42
    if ($LASTEXITCODE -ne 0) { throw "M4.2 pre-test verification failed." }
    & $python scripts\generate_m4_2_dossier.py --check
    if ($LASTEXITCODE -ne 0) { throw "M4.2 package reproduction failed." }
    & $python -m ruff check backend scripts\generate_m4_1_dossier.py scripts\generate_m4_2_dossier.py
    ...
    & $python -m app.product.verify_m42
    if ($LASTEXITCODE -ne 0) { throw "M4.2 post-test verification failed." }
} finally {
    Pop-Location
}
