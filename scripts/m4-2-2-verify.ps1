...
Push-Location $root
try {
    & $python -m app.product.verify_m422
    if ($LASTEXITCODE -ne 0) { throw "M4.2.2 pre-test verification failed." }
    ...
    & $python -m app.product.verify_m422
    if ($LASTEXITCODE -ne 0) { throw "M4.2.2 post-test verification failed." }
} finally {
    Pop-Location
}
